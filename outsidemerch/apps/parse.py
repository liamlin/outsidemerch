...
for lineup in lineups:
    ...
    for show in lineup['shows']:
        ...
        show['time_start'] = "".join(["0"+time if len(time) == 1 else time for time in show['time_start'].split(":")])
        show['time_end'] = "".join(["0"+time if len(time) == 1 else time for time in show['time_end'].split(":")])
